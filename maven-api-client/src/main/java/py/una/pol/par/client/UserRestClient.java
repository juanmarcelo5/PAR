...
public class UserRestClient {
    ...
    public ArrayList<User> getUsers() {
        ...
        WebTarget target = client.target("http://localhost:8081/parusrmcs/rest/usrmcs/users");
    ...
    public User getUserById(Integer id) {
        Client client = ResteasyClientBuilder.newClient();
        WebTarget target = client.target("http://localhost:8081/parusrmcs/rest/usrmcs/users/"+id);
        Response response = target.request().get();
     ...
     public User getUserByLoginName(String loginName) {
        Client client = ResteasyClientBuilder.newClient();
        WebTarget target = client.target("http://localhost:8081/parusrmcs/rest/usrmcs/users/login/"+loginName);
        Response response = target.request().get();
        User values = response.readEntity(User.class);
        response.close();
        return values;
